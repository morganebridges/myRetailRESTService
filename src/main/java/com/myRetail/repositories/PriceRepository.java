 ...
 */
@Repository
public interface PriceRepository extends PagingAndSortingRepository<User, Long> {

    @Query("select p from Price p where p.id=:id")
    User findUserById(@Param("id") long id);

    @Query("select u from Price u where UPPER(u.name) like UPPER(:name)")
    User findByName(@Param("name")String name);

    @Query("select u from Price u where (u.gcmId) like (:gcmId)")
